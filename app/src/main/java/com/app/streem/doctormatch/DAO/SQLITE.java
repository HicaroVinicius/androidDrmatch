...
//                + "dt_cont" + " TEXT, " + "status" + " TEXT" + ")");
        ...
        db.execSQL("create table medicosDados(id text primary key not null, endereco1 text, endereco2 text, titular text, registro text, url text,valor text,local text, dt_cont text,cartao text,dinheiro text,cheque text,plano text);");
        db.execSQL("create table consultas(id text primary key not null,KEY_CLINIC text,KEY_MEDICO text, NOME_MEDICO text, DT_AGEND text,KEY_AGEND text,HORA text, ESPECIALIDADE text,STATUS text, DT_CONT text, cpf text,valor text);");
        db.execSQL("create table dependente(id text primary key not null,nome text,idade text, sexo text, cpf text,dt_cont text);");
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("drop table IF EXISTS dependente;");
        onCreate(db);
    }
}
